<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <input type="text" max="13" name="no_hp" id="no_hp" value="{{ old('no_hp',$user->no_hp) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukkan No Hp">
        ...
        <div>
            ...
            @if ($user->role == 'admin')
                <select id="roles" name="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    ...
                    <option value="petugas-peminjam" {{ $user->role == 'petugas-peminjam' ? 'selected' : '' }}>Petugas Peminjam</option>
                </select>
            @else
        ...
        <div class="col-span-2 sm:col-span-2" id="data_poli">
            ...
            <select id="poli_id" name="poli_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="0">Pilih Poli</option>
                @foreach ($poli as $item)
                    <option value="{{ $item->id }}" {{ $item->id == $user->poli_id ? 'selected' : '' }}>{{ $item->poli_name }}</option>
                @endforeach
            </select>
        </div>
